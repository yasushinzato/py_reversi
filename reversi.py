        ...
        for x in range(8):
            ...
            if disc != SPACE:
                if disc == BLACK:
                    ...
                    color = "white"
                # 石の描画
                canvas_board.create_oval( x * CELL_PX_SIZE+4, y * CELL_PX_SIZE+4, \
                   (x+1)*CELL_PX_SIZE-4, \
                   (y+1)*CELL_PX_SIZE-4, fill=color)
           
    # 枠を描画
    for x in range(8):  # 縦線
        canvas_board.create_line(x * CELL_PX_SIZE, 0, x * CELL_PX_SIZE, BOARD_PX_SIZE, \
                                 fill="white", width=1)
    for y in range(8):  # 横線
        canvas_board.create_line(0, y * CELL_PX_SIZE, BOARD_PX_SIZE, y * CELL_PX_SIZE, \
                                 fill="black", width=1)
...
def disp_mess():
    ...
    if game.game_mode == 0:
        ...
        mess = u"対局終了 " + str(game.board.move_num-1) + u"手" + " 黒:" + str(black_discs) \
              + "　白:" + str(white_discs)
        if blackj_discs == white_discs:
            mess += u" 引き分け"
...
black_rdo0 = tkinter.Radiobutton(root, value = 0, variable = black_var, text = u"プレイヤー")  # 初期はプレイヤー
